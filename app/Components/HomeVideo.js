...
export default function AboutSection() {
  ...
  // Attempt to play on mount (must be muted for browser to allow this)
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.play()
        .then(() => setIsPlaying(true))
        .catch(() => setIsPlaying(false)); // Autoplay blocked, wait for user click
    }
  }, []);
